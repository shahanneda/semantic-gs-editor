class GizmoRenderer {
  ...
  setPlaneVertices(v1 = [], v2 = [], v3 = []) {
    console.log("called set plane verticies");
    ...
    console.log("setting plane verticies");
    ...
    const v = vec3.normalize(
      vec3.create(),
      vec3.cross(vec3.create(), normal, u)
    );
    ...
    const v4 = vec3.add(
      vec3.create(),
      center,
      vec3.scale(vec3.create(), u, scale)
    );

    this.planeVertices = new Float32Array([...v1, ...v2, ...v3, ...v4]);
    this.planeNormal = normal;
    if (this.planeNormal[2] < 0)
      vec3.scale(this.planeNormal, this.planeNormal, -1);
  }
  ...
  render() {
    ...
    gl.uniform1f(
      gl.getUniformLocation(this.program, "draw_plane"),
      drawCorners ? 0 : 1
    );
    gl.uniform1f(
      gl.getUniformLocation(this.program, "aspect"),
