...
class Camera {
  constructor({
    ...
    gl.canvas.addEventListener("mousemove", (e) => {
      ...
      this.theta -= e.movementX * 0.01 * 0.5;
      this.phi = Math.max(
        1e-6,
        Math.min(Math.PI - 1e-6, this.phi + e.movementY * 0.01 * 0.5)
      );
    ...
    gl.canvas.addEventListener("touchmove", (e) => {
      ...
      this.phi = Math.max(
        1e-6,
        Math.min(Math.PI - 1e-6, this.phi + movementY * 0.01 * 0.5)
      );
  ...
  updateKeys() {
    if (Object.values(this.keyStates).every((s) => !s) || this.disableMovement)
      return;
    ...
    if (this.keyStates.KeyW)
      vec3.add(
        this.target,
        this.target,
        vec3.scale(front, front, settings.speed)
      );
    ...
    if (this.keyStates.KeyA)
      vec3.add(
        this.target,
        this.target,
        vec3.scale(right, right, settings.speed)
      );
  ...
  update() {
    ...
    vec3.add(
      this.pos,
      this.target,
      this.getPos(this.freeFly ? 1 : this.radius)
    );
  ...
  updateWorker() {
    ...
    const dot =
      this.lastViewProjMatrix[2] * this.vpm[2] +
  ...
  raycast_gs(x, y) {
    ...
    for (let i = 0; i < gaussianCount; i++) {
      ...
      //   const alpha = opacityData[i];
      //   if (alpha < 0.1) continue;
      const dist = vec3.distance(pos, hitPos);
      if (dist < 0.5) {
        hits.push({
          id: i,
          //   dist: dist,
    ...
    // opacityData[hit.id] = 0;
    hits.forEach((hit) => {
      //   globalData.gaussians.opacities[hit.id] = 0;
      const i = hit.id;
      //   console.log(i);
    ...
    //   //   console.log(globalData.gaussians.colors[3 * i]);
    //   globalData.gaussians.colors[3 * i] = 1;
    //   globalData.gaussians.colors[3 * i + 1] = 0;
    //   globalData.gaussians.colors[3 * i + 2] = 0;
    // }
